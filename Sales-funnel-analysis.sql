SELECT * FROM `dataset-project-493315.analystics_dataset.user_events` 
WHERE event_type = 'page_view'



-- define sales funel and the different stages

WITH funel_stages AS (
...
FROM `dataset-project-493315.analystics_dataset.user_events`

--WHERE event_date >= TIMESTAMP(DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY))

)

SELECT * FROM funel_stages

-- conversion rate throung the funel
WITH funel_stages AS (
...
SELECT
  ...
  ROUND(AVG(TIMESTAMP_DIFF(purchase_time, views_time, MINUTE)), 2) AS avg_total_journey_minutes
...
total_visitors,
total_buyers,
ROUND(total_revenue, 2) AS revenue_total,
total_orders,
